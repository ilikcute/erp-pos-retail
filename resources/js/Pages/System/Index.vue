...
const props = defineProps({
    ...
    documentTypes: { type: Object, default: () => ({ data: [] }) },
    ...
    <DashboardLayout>
        <div class="mb-6">
            ...
            <p class="text-ink-secondary text-sm">
                Kelola pengguna, peranan (roles), konfigurasi sistem, format
                dokumen, profil perusahaan, dan audit log.
            </p>
        </div>

        <!-- Tabs -->
        <div
            class="flex space-x-1 mb-6 border-b border-border-soft overflow-x-auto whitespace-nowrap scrollbar-none"
        >
            <button
                @click="
                    activeTab = 'users';
        ...
        <!-- Search Bar -->
        <div
            v-if="activeTab === 'users' || activeTab === 'roles'"
            class="mb-6 flex items-center gap-md max-w-md bg-surface-card rounded-lg border border-border-soft p-sm shadow-soft"
        >
            <svg
                class="w-5 h-5 text-ink-muted ml-sm"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            >
        ...
        <div v-if="activeTab === 'roles'">
            <DataTable :columns="roleColumns" :rows="roles">
                <template #cell-name="{ value }">
                    <span class="font-semibold text-ink-primary font-mono">{{
                        value
                    }}</span>
                </template>
                <template #cell-status="{ row }">
        ...
        <div v-if="activeTab === 'settings'" class="space-y-xl">
            <form
                ...
                class="space-y-lg bg-surface-card rounded-lg border border-border-soft p-xl shadow-soft"
            >
                <h2 class="text-lg font-bold text-ink-primary border-b pb-sm">
                    Pengaturan Sistem ⚙️
                </h2>

                <div class="space-y-md">
                    <div
                        v-for="(setting, idx) in settingsForm.settings"
                        :key="setting.id"
                        class="grid grid-cols-1 md:grid-cols-3 gap-md items-center py-sm border-b border-border-soft last:border-b-0"
                    >
                        <div>
                            <span
                                class="font-mono text-xs font-bold text-brand uppercase block"
                                >{{ setting.key }}</span
                            >
                ...
                <div class="flex justify-end pt-md border-t border-border-soft">
                    <BaseButton type="submit" :loading="settingsForm.processing"
                        >💾 Simpan Konfigurasi</BaseButton
                    >
                </div>
            </form>
        </div>
        ...
        <div v-if="activeTab === 'document-types'">
            <DataTable
                ...
                :rows="documentTypes.data"
                :paginated="false"
                :meta="documentTypes"
            >
                <template #cell-code="{ value }">
                    <span
                        class="font-mono text-xs font-bold text-ink-primary"
                        >{{ value }}</span
                    >
                </template>
                <template #cell-status="{ row }">
        ...
        <div
            v-if="activeTab === 'business-profile'"
            class="bg-surface-card rounded-lg border border-border-soft p-xl max-w-4xl shadow-soft"
        >
            <form @submit.prevent="submitProfile" class="space-y-xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-xl">
                    <FormInput
                        ...
                        required
                        :error="profileForm.errors.business_name"
                    />
                    <FormInput
                        label="Nama Legal"
                        v-model="profileForm.legal_name"
                        :error="profileForm.errors.legal_name"
                    />
                    <FormInput
                        label="NPWP (Tax ID)"
                        v-model="profileForm.tax_id"
                        :error="profileForm.errors.tax_id"
                    />
                    <FormInput
                ...
                <div class="flex justify-end pt-md border-t border-border-soft">
                    <BaseButton type="submit" :loading="profileForm.processing"
                        >💾 Simpan Profil Bisnis</BaseButton
                    >
                </div>
            </form>
        </div>
        ...
        <div v-if="activeTab === 'audit'">
            ...
            >
                ...
                <template #cell-module="{ value }">
                    <span
                        class="px-2 py-0.5 bg-surface-subtle text-ink-secondary rounded text-xs uppercase font-semibold"
                    >
                        {{ value }}
                    </span>
                </template>
